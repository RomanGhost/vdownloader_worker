...
//
// Download jobs are no longer submitted over HTTP: they arrive on the
// "video.jobs" RabbitMQ queue and are handled by HandleJobMessage.
package api
...
// Server handles REST requests for the frontend and job requests from RabbitMQ.
type Server struct {
	db        *storage.DB
	outDir    string
	publishFn func(ctx context.Context, body []byte) error // publishes a completion notification
	log       *slog.Logger
}

// completedMessage is the JSON body published to the "video.completed" queue:
// just the finished job's file_id. Consumers fetch the full outcome (ready or
// failed, download URL, error) via GET /api/jobs/{file_id}.
type completedMessage struct {
	FileID string `json:"file_id"`
}

// New creates a Server. publishFn is called with a completion notification on
// every finished job (ready or failed); consumers fetch the outcome via
...
// ── job requests (RabbitMQ) / GET /api/jobs ──────────────────────────────────

// jobRequest is the JSON value read from the "video.jobs" queue. FileID is
// generated by the requester so it can correlate the eventual completion
// notification (which carries only file_id) back to this job.
//
...
// HandleJobMessage processes a single job request received from RabbitMQ: it
// persists the job, then runs the download and publishes completion.
// Called from the mq consumer loop, so it never writes an HTTP response.
func (s *Server) HandleJobMessage(ctx context.Context, value []byte) {
	...
	// Run the download in the background so the mq consumer loop can keep
...
// publishes a completion notification so consumers can fetch the outcome.
func (s *Server) runDownload(ctx context.Context, record *storage.Download, req downloader.Request) {
	...
	body, err := json.Marshal(completedMessage{FileID: record.FileID})
	if err != nil {
		s.log.Error("api: marshal completion", "job_id", record.ID, "file_id", record.FileID, "err", err)
		return
	}
	if err := s.publishFn(ctx, body); err != nil {
